"""
Premium City Box Office Report Generator
Same UI/styling as the States report — city-level data only.
Output: 1280px wide
"""
...
def aggregate(all_results):
    ...
    for r in all_results:
        ...
        if city not in city_stats:
            city_stats[city] = {"city": city, "gross":0,"tickets":0,"seats":0,"shows":0,"venues":set()}
        cs = city_stats[city]
        cs["gross"]+=g; cs["tickets"]+=t; cs["seats"]+=s; cs["shows"]+=1; cs["venues"].add(venue)

    out = []
    for v in city_stats.values():
    ...
    for ri, row in enumerate(rows):
        ...
        for ci, (cell, cw, al) in enumerate(zip(row, col_widths, alignments)):
            if isinstance(cell, dict) and cell.get("type") == "occ_bar":
                ...
                if is_rank:
                    font  = F_TBL_RANK;  color = MUTED
                elif is_name:
                    font  = F_TBL_NAME;  color = TEXT
                elif is_gross:
                    font  = F_TBL_GROSS; color = ACCENT
                else:
                    font  = F_TBL_NUM;   color = TEXT
...
def generate_premium_city_image_report(all_results, output_path,
    ...
    TOP_ROWS  = 20   # show more cities since no state table
    city_list = aggregate(all_results)
    ...
    col_headers = ["#", "CITY",  "VENS", "SHOWS", "TICKETS", "OCCUPANCY", "GROSS"]
    ...
    def render(canvas_h):
        ...
        draw.text((PAD, 18), "CITY-WISE BOX OFFICE REPORT", font=F_EYEBROW, fill=ACCENT)
        ...
        PILL_H = 40
        PILL_R = 7
        pill_items = [show_date, f"{num_cities} Cities", f"{num_venues} Theatres", f"{num_shows} Shows"]
        mx = PAD
        ...
        sub_off = (th_font(draw, F_SEC_TITLE) - th_font(draw, F_SEC_SUB)) // 2
        draw.text((sub_x, sec_y+sub_off+2), f"TOP {min(TOP_ROWS, len(city_list))} \u00b7 BY GROSS", font=F_SEC_SUB, fill=MUTED)
        y = sec_y + th_font(draw, F_SEC_TITLE, stroke=1) + 16
        draw.line([(PAD, y), (W-PAD, y)], fill=BORDER, width=1)
        y += 18
        ...
        for i, c in enumerate(city_list[:TOP_ROWS], 1):
            city_rows.append([
                str(i), c["city"],
                str(c["venues"]), str(c["shows"]),
                f"{c['tickets']:,}",
                {"type": "occ_bar", "value": c["occupancy"]},
                fmt(c["gross"]),
            ])

        y = draw_table(draw, y, col_headers, city_rows, col_widths, col_aligns, PAD)

        # ── FOOTER ───────────────────────────────────────────────────────────
        y += 10
        FOOT_H = 95
        draw.rectangle([0, y, W, y+FOOT_H], fill=SURFACE)
        f1 = "Generated by CinePulseBO  \u00b7  Data: BookMyShow & District" + datetime.now().strftime("%d %b %Y, %I:%M %p")
...
if __name__ == "__main__":
    ...
    cities_venues = {
        ...
        "Mumbai":        ["PVR Juhu", "INOX Nariman", "Cinepolis Andheri"],
        "Delhi":         ["PVR Select", "INOX Nehru", "Cinepolis Vasant"],
        "Pune":          ["PVR Phoenix", "INOX FC Road", "Cinepolis Pune"],
        "Kolkata":       ["PVR Mani", "INOX South City", "Cinepolis Kolkata"],
        "Ahmedabad":     ["PVR Acropolis", "INOX Alpha", "Cinepolis Ahmedabad"],
        "Surat":         ["PVR VR Mall", "INOX Surat", "Cinepolis Surat"],
        "Jaipur":        ["PVR Crystal", "INOX GT Central", "Cinepolis Jaipur"],
        "Lucknow":       ["PVR Rave", "INOX Fun", "Cinepolis Lucknow"],
        "Chandigarh":    ["PVR Centra", "INOX Piccadilly", "Cinepolis Chandigarh"],
    }

    random.seed(42)
    sample_data = []
    for city, venues in cities_venues.items():
        for venue in venues:
    ...
    generate_premium_city_image_report(
        sample_data,
        output_path="city_report.png",
        movie_name="DHURANDHAR THE REVENGE",
        show_date="18 Mar 2026"
    )